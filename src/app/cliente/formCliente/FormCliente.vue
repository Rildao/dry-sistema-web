...
import {formatarValorReal} from "../../../utils";
...
export default {
  ...
  methods: {
    onRowEditSave(event) {
      ...
      this.lancamentos[index] = newData;
    },
    formatarValorReal,
    cadastrar() {
      this.v$.$touch();
...
<template>
  <div class="card">
    <Fieldset>
      ...
      <div class="grid">
        <div class="field col-12 lg:col-9 md:col-9">
          ...
          <InputText id="nome" @input="v$.nome.$touch()" :class="{ 'p-invalid': nome !== null && v$.nome.$error }"
                     class="w-full" v-model="nome" type="text" placeholder="Nome"/>
        ...
        <div class="field col-12 lg:col-3 md:col-3">
          ...
          <InputMask id="cpf" @input="v$.cpf.$touch()" :class="{ 'p-invalid': cpf !== null && v$.cpf.$error }"
                     class="w-full" v-model="cpf" type="text" mask="999.999.999-99" placeholder="999.999.999-99"/>
    ...
    <Fieldset class="mt-5" v-if="rotaClienteEditar">
      ...
      <DataTable v-model:filters="filters" v-model:selection="vendaSelecionada" paginator :rows="5"
                 :rowsPerPageOptions="[5, 10, 15]" :value="vendas" selectionMode="single" dataKey="id"
                 :metaKeySelection="true" tableStyle="min-width: 50%">
        <template #header>
    ...
    <Dialog v-model:visible="modal" modal :closable="false" :style="{ width: '85%' }">
      <Fieldset>
        ...
        <div class="formgrid grid">
          <div class="field col-12 lg:col-4 md:col-4">
            ...
            <Dropdown id="tipoVenda" v-model="venda.tipoVenda" :options="tipoVendaEnum" optionLabel="nome"
                      optionValue="codigo" showClear placeholder="Selecione o Tipo" class="w-full"
                      :disabled="atualizaVenda"/>
          </div>

          <div class="field col-12 lg:col-4 md:col-4">
            <label for="valorVenda">Valor</label>
            <InputNumber id="tipoVenda" v-model="venda.valorVenda" optionLabel="nome" optionValue="codigo" showClear
                         placeholder="R$ 0,00" class="w-full" mode="currency" currency="BRL" :disabled="atualizaVenda"/>
          </div>

          <div class="field col-12 lg:col-4 md:col-4">
            <label for="quantidadeParcelas">Quant. de Parcelas <i class="pi pi-info-circle"
                                                                  v-tooltip="`Quant. de Parcelas aplicasse apenas no tipo 'Crediário'.`"
                                                                  style="color: slateblue"></i></label>
            <InputNumber v-if="venda.tipoVenda === 'CREDIARIO'" id="quantidadeParcelas" class="w-full"
                         v-model="venda.quantidadeParcelas" inputId="minmax-buttons" mode="decimal" showButtons :min="0"
                         :disabled="atualizaVenda"/>

            <InputNumber v-else id="quantidadeParcelas" class="w-full" inputId="minmax-buttons" mode="decimal"
                         showButtons :min="0" disabled/>
          </div>

          <div class="field col-12 lg:col-4 md:col-4">
            <label for="dataVenda">Data da Venda</label>
            <Calendar id="dataVenda" class="w-full" v-model="venda.dataVenda" showIcon placeholder="dd/mm/aaaa"
                      dateFormat="dd/mm/yy" :disabled="atualizaVenda"/>
          </div>

          <div class="field col-12 lg:col-4 md:col-4">
            <label for="dataVencimentoLancamento">
              Dia de Venc. do Pagamento
              <i class="pi pi-info-circle" v-tooltip="`Dia de Venc. do Pagamento aplicasse apenas no tipo 'Crediário'.`"
                 style="color: slateblue"></i>
            </label>
            <InputNumber v-if="venda.tipoVenda === 'CREDIARIO'" id="dataVencimentoLancamento" class="w-full"
                         v-model="venda.diaVencimentoLancamento" :min="1" :max="30" :disabled="atualizaVenda"/>
          ...
          <div class="field col-12 lg:col-12 md:col-12">
            ...
            <Editor v-model="venda.descricao" id="abservacao" editorStyle="height: 180px"
                    aria-describedby="editor-error" in>
              <template #toolbar>
                                <span class="ql-formats">
                                    <button v-tooltip.bottom="'Bold'" class="ql-bold"></button>
                                    <button v-tooltip.bottom="'Italic'" class="ql-italic"></button>
                                    <button v-tooltip.bottom="'Underline'" class="ql-underline"></button>
                                    <button v-tooltip.bottom="'Ordered List'" class="ql-list ql-active"
      ...
      <Fieldset v-if="exibirLancamento">
        ...
        <DataTable v-model:editingRows="editingRows" :value="lancamentos" editMode="row" dataKey="id"
            ...
            <template #editor="{ data, field }">
              <Dropdown v-model="data[field]" :options="statusVendaEnum" optionLabel="nome" optionValue="codigo" placeholder="Selecione o status">
                <template #option="slotProps">
                  <Tag :value="slotProps.option.value" :severity="formatarEnumStatus(slotProps.option.value)" />
                </template>
              </Dropdown>
            </template>
            <template #body="slotProps">
              <Tag :value="slotProps.data.statusVenda" :severity="formatarEnumStatus(slotProps.data.statusVenda)" />
            </template>
          </Column>
